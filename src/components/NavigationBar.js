      ...
      <Search>
        ...
        <StyledInputBase
          ...
          onInput={(e) => {
            props.dispatch({
              type: "setSearch",
              payload: e.currentTarget.firstChild.value,
            });
          }}
        />
      </Search>
  ...
  if (location.pathname === "/login" || location.pathname === "/signup") {
    return (
      <Box
        sx={{
          typography: "body1",
          "& > :not(style) + :not(style)": { ml: 2, mr: 2 },
        }}
      >
        <Link color="inherit" variant="body2" href="/login">
          Login
        </Link>
        <Link color="inherit" variant="body2" href="/signup">
          Sign up
        </Link>
      </Box>
    );
...
export default function NavigationBar() {
  ...
  if (localStorage.getItem("admin@upgrad.com") === null) {
    const adminDetails = {
      contactNumber: "+9112345676",
      email: "admin@upgrad.com",
      firstName: "Renu Bhati",
      isAdmin: true,
      lastName: "admin",
      password: "pass@admin",
    };
